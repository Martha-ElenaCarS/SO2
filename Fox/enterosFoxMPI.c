...
#define Order(A) ((A)->n_bar)
    int  entries[MAX];	/* Elementos de la matriz local	*/
...
/*Inicializa la estructura de comunicación de MPI entre los procesos. Ninguna función de comunicación MPI funcionará sin esta instrucción al inicio.*/
    MPI_Init(&argc, &argv);/*Determina el rango (identificador) del proceso que lo llama dentro del comunicador seleccionado.*/
    ...
    MPI_Bcast(&n, 1, MPI_INT, 0, MPI_COMM_WORLD); /*Envía un mensaje desde un proceso origen a todos los procesos pertenecientes al mismo grupo.*/
    n_bar = n/grid.q;
...
void Read_matrix(
    ...
    if (grid->my_rank == 0) {
        temp = (int*) malloc(Order(local_A)*sizeof(int));
        //printf("%s\n", prompt);
        fflush(stdout);
            ...
            for (grid_col = 0; grid_col < grid->q; grid_col++) {
                ...
                if (dest == 0) {
                    for (mat_col = 0; mat_col < Order(local_A); mat_col++) {
                        *((local_A->entries)+mat_row*Order(local_A)+mat_col) = 1 + rand()%5;
						//scanf("%f", (local_A->entries)+mat_row*Order(local_A)+mat_col);
					}
                } else {
                    for (mat_col = 0; mat_col < Order(local_A); mat_col++) {
                        *(temp + mat_col) = 1 + rand()%5;
						//scanf("%f", temp + mat_col);
					}
                    MPI_Send(temp, Order(local_A), MPI_INT, dest, 0, grid->comm);
                }
            }
        }
        free(temp);
        ...
        for (mat_row = 0; mat_row < Order(local_A); mat_row++) 
            MPI_Recv(&Entry(local_A, mat_row, 0), Order(local_A), MPI_INT, 0, 0, grid->comm, &status);
    }
...
void Print_matrix(
    ...
    if (grid->my_rank == 0) {
        ...
        for (mat_row = 0;  mat_row < n; mat_row++) {
            ...
            for (grid_col = 0; grid_col < grid->q; grid_col++) {
                ...
                if (source == 0) {
                    ...
                    MPI_Recv(temp, Order(local_A), MPI_INT, source, 0, grid->comm, &status);
                    for(mat_col = 0; mat_col < Order(local_A); mat_col++)
        ...
        for (mat_row = 0; mat_row < Order(local_A); mat_row++) 
            MPI_Send(&Entry(local_A, mat_row, 0), Order(local_A), MPI_INT, 0, 0, grid->comm);
    }
...
void Build_matrix_type(LOCAL_MATRIX_T*  local_A ) {
    ...
    MPI_Type_contiguous(Order(local_A)*Order(local_A), MPI_INT, &temp_mpi_t);
