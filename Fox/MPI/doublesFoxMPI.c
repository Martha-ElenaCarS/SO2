...
#define Order(A) ((A)->n_bar)
    double  entries[MAX];	/* Elementos de la matriz local	*/
...
int main(int argc, char* argv[]) {
    ...
    MPI_Init(&argc, &argv); 			/*Inicializa la estructura de comunicación de MPI entre los procesos. Ninguna función de comunicación MPI funcionará sin esta instrucción al inicio.*/
    MPI_Comm_rank(MPI_COMM_WORLD, &my_rank); 	/*Determina el rango (identificador) del proceso que lo llama dentro del comunicador seleccionado.*/
    ...
    MPI_Bcast(&n, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD); 	/*Envía un mensaje desde un proceso origen a todos los procesos pertenecientes al mismo grupo.*/
    n_bar = n/grid.q;
    ...
    Order(local_C) = n_bar;
	start = MPI_Wtime();
    Fox(n, &grid, local_A, local_B, local_C);		/*Funcion Principal de Fox*/
    double memoria = obtener_memoria();
	
    end = MPI_Wtime();					/*Tiempo en segundos desde un tiempo arbitrario en el pasado.*/
    ...
    if(file1 != NULL){
        fwrite(&tiempo,sizeof(double), 1, file1);	/*Escritura de resultados de tiempo en los archivos*/
    }
    fflush(file1);
    fclose(file1);

    FILE *file2;
    file2 = fopen("datosMemoria.dat","ab+");		/*Escritura de resultados de Memoria en los archivos*/	
    if(file2 != NULL){
    ...
    MPI_Finalize();					/*Finaliza Ejecucion de MPI*/
}  /* main */

long obtener_memoria() {				/*Deinicion del tipo de datos*/
...
/*********************************************************/
void Fox(					/*Funcion Principal de Fox*/
        int              n         , 
        GRID_INFO_T*     grid      , 
        LOCAL_MATRIX_T*  local_A   ,
        LOCAL_MATRIX_T*  local_B   ,
